...
/**
 * Immutable scroll choices for one G gesture: the bag's mounts and nothing else (the quick
 * slot is H's, never G's).
 * AF: each mount followed by its explicit deposit action when occupied, then the held-stack
 * deposit when a bag and held stack exist.
 * RI: one to four mounts, no action for an empty mount, and no mutable list escapes.
 */
public final class GearChoices {
    private GearChoices() {}
    public enum Kind { MOUNT, STOW_MOUNT, STOW_HELD }
     ...
     * throws: IllegalArgumentException for no mounts (nothing to choose from: G does nothing
     * without a bag), more than four, or occupancy bits beyond them.
     */
    public static List<Choice> build(int mounts, int occupied, boolean held) {
        if (mounts < 1 || mounts > 4 || occupied < 0 || occupied >= (1 << mounts))
        ...
        if (held) choices.add(new Choice(Kind.STOW_HELD, -1));
        return List.copyOf(choices);
    }

    /**
     * effects: returns the index the highlight starts on when a gesture opens: the first backpack
     * mount, so holding G and releasing takes what is in the first slot without a scroll. A
     * deposit is never the default, so releasing without scrolling never puts anything in the bag.
