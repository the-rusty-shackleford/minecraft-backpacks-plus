...
final class RenderedBounds implements VertexConsumer {
    record Shape(ModelBounds bounds, boolean sprite) {}
    ...
    static Shape measure(AbstractClientPlayer player, ItemStack stack, int light) {
        ...
        try {
            ...
            ModelBounds bounds = new ModelBounds(vertices.minX, vertices.minY, vertices.minZ, vertices.maxX, vertices.maxY, vertices.maxZ);
            return new Shape(bounds, bounds.squareSprite() && !vertices.rotatedFaces);
        } catch (RuntimeException unsupported) {
            LogUtils.getLogger().warn("Backpacks+ could not measure {}: {}. Placement override may be needed.",
                    stack.getItem(), unsupported.toString());
            return new Shape(new ModelBounds(-0.5, -0.5, -0.5, 0.5, 0.5, 0.5), false);
        }
    }
